...
// Enhanced palette items for professional quotations
const paletteItems = [
  { type: 'header', label: 'Header Section', icon: TextIcon, description: 'Company logo and title' },
  ...
  { type: 'terms', label: 'Terms & Conditions', icon: FileText, description: 'Add terms and conditions' },
...
// Sample data for table preview matching your quotation format
const sampleTableData = [
  { sr: '1', capacity: '100MT', jobType: 'Loading work', duration: '01 Month', rental: '7,85,000', mob: '10,000/-', demob: '10,000/-' },
  { sr: '2', capacity: 'Demag 60MT', jobType: 'Unloading work', duration: '01 Month', rental: '3,20,000', mob: 'Included', demob: 'Included' }
];

// Enhanced field options matching your quotation format
const fieldOptions = [
  // Company Details
  ...
  { value: 'company_gst', label: 'Company GST Number' },
  ...
  { value: 'kind_attn', label: 'Kind Attention' },
  ...
  { value: 'equipment_capacity', label: 'Equipment Capacity' },
  ...
  const [{ isOver }, drop] = useDrop(() => ({
    ...
    drop: (item: { elementType: string }, monitor) => {
      ...
      const newElement: TemplateElement = {
        id: `element-${Date.now()}`,
        type: item.elementType as 'text' | 'field' | 'table' | 'image' | 'spacer' | 'header' | 'terms',
        content: '',
        fieldType: undefined
      };
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left font-medium">Rental</th>
                    <th className="px-3 py-2 text-left font-medium">Mob.</th>
                    <th className="px-3 py-2 text-left font-medium">De-Mob</th>
                  </tr>
                </thead>
                <tbody>
                  {sampleTableData.map((row, idx) => (
                    <tr key={idx} className="border-t">
                      ...
                      <td className="px-3 py-2 font-medium">{row.rental}</td>
                      <td className="px-3 py-2">{row.mob}</td>
                      <td className="px-3 py-2">{row.demob}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div>
            <div className="mb-3">
              ...
              <textarea
                className="w-full p-3 border border-gray-300 rounded-md resize-vertical font-mono text-sm"
                rows={3}
                value={element.content || 'QUOTATION\n\nASP CRANES - AADISHAKTI PROJECTS'}
                onChange={(e) => handleContentChange(e.target.value)}
                placeholder="Enter header content..."
              />
            </div>
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-md">
              <div className="text-xs text-blue-600 mb-2">Preview:</div>
              <div className="text-center font-bold text-lg border-b pb-2">
                {element.content?.split('\n').map((line, idx) => (
                  <div key={idx} className={idx === 0 ? 'text-xl' : 'text-base'}>
                    {line || 'Header Line'}
                  </div>
                )) || 'QUOTATION'}
              </div>
          ...
          <div>
            <div className="mb-3">
              ...
              <textarea
                className="w-full p-3 border border-gray-300 rounded-md resize-vertical font-mono text-sm"
                rows={8}
                value={element.content || `COMMERCIAL TERMS

a. Only Electronic Fund Transfer is Acceptable, bank details mentioned below.

b. Marching and Deal Confirmation: 50% of Monthly Rent with GST and Full mobilization charges

c. Compliance: GST and any other government taxes as applicable.

d. Force Majeure: Any interruption/delay in work shall not exempt "The Hirer" from paying daily rental.`}
                onChange={(e) => handleContentChange(e.target.value)}
                placeholder="Enter terms and conditions..."
              />
            </div>
            <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
              <div className="text-xs text-yellow-600 mb-2">Preview:</div>
              <div className="text-sm whitespace-pre-wrap">
                {element.content || 'Terms and conditions will appear here...'}
              </div>
            </div>
          </div>
        );
...
export default function TemplateBuilder() {
  ...
  const addElement = (type: 'text' | 'field' | 'table' | 'image' | 'spacer' | 'header' | 'terms') => {
    const newElement: TemplateElement = {
      id: `element-${Date.now()}`,
      type,
      content: '',
            ...
            <table className="w-full text-xs border border-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-2 py-1 text-left border">Capacity</th>
                  <th className="px-2 py-1 text-left border">Job Type</th>
                  <th className="px-2 py-1 text-left border">Duration</th>
                  <th className="px-2 py-1 text-left border">Rental</th>
                </tr>
              </thead>
              <tbody>
                {sampleTableData.slice(0, 2).map((row, idx) => (
                  <tr key={idx}>
                    <td className="px-2 py-1 border">{row.sr}</td>
                    <td className="px-2 py-1 border">{row.capacity}</td>
                    <td className="px-2 py-1 border">{row.jobType}</td>
                    <td className="px-2 py-1 border">{row.duration}</td>
                    <td className="px-2 py-1 border font-medium">{row.rental}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        return (
          <div className="text-center border-b border-gray-300 pb-4 mb-4">
            <div className="font-bold text-lg">
              {element.content?.split('\n').map((line, idx) => (
                <div key={idx} className={idx === 0 ? 'text-xl mb-2' : 'text-base'}>
                  {line || 'Header Line'}
                </div>
              )) || 'QUOTATION'}
            </div>
        ...
        return (
          <div className="mt-4 p-3 bg-gray-50 border border-gray-200 rounded">
            ...
            <div className="text-xs whitespace-pre-wrap">
              {element.content?.substring(0, 200) || 'Terms and conditions will appear here...'}
              {element.content && element.content.length > 200 && '...'}
            </div>
          </div>
        );
      case 'spacer':
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
              ...
              <div className="space-y-2">
                <button
                  ...
                  className="w-full px-3 py-2 text-left bg-blue-50 hover:bg-blue-100 rounded-md text-sm flex items-center gap-2"
                >
                  <Plus size={14} />
                  Add Header
                </button>
                ...
                <button
                  onClick={() => addElement('terms')}
                  className="w-full px-3 py-2 text-left bg-yellow-50 hover:bg-yellow-100 rounded-md text-sm flex items-center gap-2"
                >
                  <Plus size={14} />
                  Add Terms
                </button>
