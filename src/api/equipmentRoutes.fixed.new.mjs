...
const mapDbRowToEquipment = (row) => {
  try {
    ...
    // Map database fields to frontend model, providing defaults for missing fields
    const mapped = {
      ...
      manufacturingDate: '', // Not in actual database schema, but needed by frontend
      registrationDate: '', // Not in actual database schema, but needed by frontend
      maxLiftingCapacity: 0, // Not in actual database schema, but needed by frontend
      unladenWeight: 0, // Not in actual database schema, but needed by frontend
      baseRates: {
        micro: baseMicro,
        small: baseSmall,
        monthly: baseMonthly,
      ...
      runningCostPerKm: 0, // Not in actual database schema, but needed by frontend
      description: row.description || '', // Might exist in schema
...
router.post('/', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Insert new equipment - ADJUSTED to only include fields that actually exist in the database
...
router.put('/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Update equipment - ADJUSTED to only include fields that actually exist in the database
    const result = await client.query(`
