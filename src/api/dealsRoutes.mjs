...
const pool = new pg.Pool({
  ...
  password: process.env.VITE_DB_PASSWORD || 'vedant21',  // Using the original password
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const deals = result.rows.map(row => ({
      ...
      probability: 50, // Default since probability field doesn't exist
  ...
  try {
    ...
    if (!validStages.includes(stage)) {
      ...
      return res.status(400).json({ 
        error: `Invalid stage value. Must be one of: ${validStages.join(', ')}` 
      });
    }
    
    client = await pool.connect();
      // Check if deal exists before update
    const checkQuery = 'SELECT deal_id FROM deals WHERE deal_id = $1';
    const checkResult = await client.query(checkQuery, [id]);
    
    if (checkResult.rows.length === 0) {
      console.error(`Deal ${id} not found`);
      return res.status(404).json({ error: `Deal with id ${id} not found` });
    }
    
    console.log(`Deal ${id} found, proceeding with update`);
    
    // Update the deal stage (stored as status field in DB)
    const updateResult = await client.query(
      'UPDATE deals SET status = $1, updated_at = NOW() WHERE deal_id = $2 RETURNING deal_id, status',
      [stage, id]
    );
